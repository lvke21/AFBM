...
- Admin nutzt Firebase Auth Custom Claims und serverseitig verifizierte ID Tokens.
...
| `E2E_FIREBASE_ADMIN_ID_TOKEN` | optional fuer lokale Admin-Smokes | nicht gesetzt | nicht gesetzt |
...
Staging nutzt `apphosting.yaml` mit Firebase Runtime-Konfiguration. Production soll eine eigene App-Hosting-Konfiguration oder einen eigenen Backend-Branch mit denselben Regeln nutzen:
...
- Secret Manager fuer benoetigte serverseitige Secrets wie `DATABASE_URL`
...
npx tsc --noEmit
npm run lint
npm test -- --run src/lib/env/runtime-env.test.ts src/app/api/admin/online/actions/route.test.ts
```
