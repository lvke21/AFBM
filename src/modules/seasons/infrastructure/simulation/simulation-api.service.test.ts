...
describe("simulation API service", () => {
  ...
  it("replays game simulation from explicit RNG instances with the same seed", () => {
    const input = validCreateGameInput("api-rng-replay-1");
    const firstGame = createGame(input);

    simulateGame(firstGame.gameId, createRng(firstGame.seed));
    const firstResult = getGameResult(firstGame.gameId);

    resetSimulationApiStoreForTests();

    const secondGame = createGame(input);

    simulateGame(secondGame.gameId, createRng(secondGame.seed));
    const secondResult = getGameResult(secondGame.gameId);
