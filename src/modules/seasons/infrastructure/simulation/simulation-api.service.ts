...
import { createRng, createSeededId, type SeededRng } from "@/lib/random/seeded-rng";
...
export function createGame(input: CreateSimulationGameInput) {
  ...
  const gameId =
    ...
    createSeededId(
      "simulation-game",
      `simulation-api:${createdAt}:${parsed.data.teams.home.id}:${parsed.data.teams.away.id}:${parsed.data.seed ?? ""}`,
      12,
    );
...
export function simulateGame(
  ...
  const game = readGame(gameId);
  const simulationRng = rng ?? createRng(game.seed);
  const simulatedAt = new Date().toISOString();
  const result =
    game.result ??
    generateMatchStats(
      game.context,
      simulationRng.fork(`match:${game.context.matchId}:${game.seed}`),
    );
