...
import { getActiveDataBackend, getRepositories } from "./index";
...
describe("repository provider", () => {
  ...
  it("uses firestore as the primary backend when emulator config is available", () => {
  ...
  it("keeps prisma as an explicit legacy fallback", () => {
    vi.stubEnv("DATA_BACKEND", "prisma");

    const repositories = getRepositories();

    expect(repositories.backend).toBe("prisma");
    expect(getActiveDataBackend()).toBe("prisma");
  });

  it("falls back to prisma when no backend is configured and firestore is unavailable", () => {
    vi.stubEnv("DATA_BACKEND", "");
    vi.stubEnv("FIRESTORE_EMULATOR_HOST", "");
    vi.stubEnv("FIREBASE_EMULATOR_HOST", "");
    vi.stubEnv("FIREBASE_PROJECT_ID", "");
    vi.stubEnv("NEXT_PUBLIC_FIREBASE_PROJECT_ID", "");
    vi.stubEnv("FIRESTORE_PREVIEW_DRY_RUN", "false");
    vi.stubEnv("FIRESTORE_PREVIEW_ALLOWLIST_PROJECTS", "");
