...
const BASELINE = {
  pointsPerGame: 48.0,
  totalYardsPerGame: 909.1,
  puntsPerTeamGame: 4.95,
  fgAttemptsPerTeamGame: 2.84,
  touchdownsPerTeamGame: 2.67,
  turnoverOnDownsPerTeamGame: 2.84,
} as const;
...
function createMatchContext(index: number): SimulationMatchContext {
  return {
    matchId: `aggressive-softfail-match-${index + 1}`,
    saveGameId: "save-1",
    seasonId: "season-1",
    kind: "REGULAR_SEASON",
    simulationSeed: `aggressive-softfail-seed-${index + 1}`,
    seasonYear: 2026,
    week: (index % 10) + 1,
...
function simulateBatch(gameCount: number) {
  ...
  const corridorOutcome = new Map<string, number>();
  const puntByZone = new Map<string, number>();
  const fgByZone = new Map<string, number>();
  ...
  for (let index = 0; index < gameCount; index += 1) {
    ...
    for (const drive of result.drives) {
      ...
      if (drive.targetedAggressiveGoForIt) {
        ...
        increment(
          corridorOutcome,
          `${zone} :: ${drive.aggressiveGoForItResolution ?? "UNKNOWN"} :: ${drive.resultType}`,
        );
        if (drive.postFourthDownConverted) {
          targetedSuccesses += 1;
        }
        if ((drive.softFailCount ?? 0) > 0) {
          softFails += 1;
        }
  ...
  return {
    ...
    targetedAttempts,
    targetedSuccessRate: targetedAttempts === 0 ? 0 : targetedSuccesses / targetedAttempts,
    softFailRate: targetedAttempts === 0 ? 0 : softFails / targetedAttempts,
    corridorOutcomeRows: rowsFromMap(corridorOutcome),
    todZoneRows: rowsFromMap(todByZone),
    puntZoneRows: rowsFromMap(puntByZone),
    fgZoneRows: rowsFromMap(fgByZone),
    sampleDrives,
  };
}
...
function main() {
  ...
  const recommendation =
    ...
    result.fgAttemptsPerTeamGame <= BASELINE.fgAttemptsPerTeamGame
      ? "Der gezielte Soft-Fail-Pfad verbessert den Korridor. Als naechstes sollte nur noch die Rest-TOD-Last in OPP35_TO_20 angefasst werden."
      : "Der Soft-Fail-Pfad reduziert TOD noch nicht genug oder verschiebt die Rate zu stark. Als naechstes sollte nur die Post-Convert-Stabilisierung in diesem Korridor weiter verfeinert werden.";

  const html = `<!doctype html>
<html lang="de">
<head>
  <meta charset="utf-8" />
  <title>Aggressive Soft Fail Report ${REPORT_DATE}</title>
  <style>
...
<body>
  <section>
    <h1>Aggressive Soft Fail Report</h1>
    <p>50-Spiele-Validierung fuer AGGRESSIVE Coaches in OPP45_TO_35 bei 4th & 3-5 mit Conversion-Boost, Soft-Fail und Post-Convert-Stabilisierung.</p>
  ...
  <section>
    <h2>Korridor-Erfolg</h2>
    ${renderTable(
      ["Kennzahl", "Wert"],
      [
        ["Targeted Attempts", String(result.targetedAttempts)],
        ["GO_FOR_IT Erfolgsrate", `${round(result.targetedSuccessRate * 100, 1)}%`],
        ["Soft-Fail-Anteil", `${round(result.softFailRate * 100, 1)}%`],
      ],
    )}
  </section>
  <section>
  ...
  <section>
    <h2>Korridor-Outcomes</h2>
    ${renderTable(["Zone :: Resolution :: Folge-Outcome", "Count", "Anteil %"], result.corridorOutcomeRows)}
  </section>
  <section>
  ...
  <section>
    ...
    ${renderTable(["Zone", "Count", "Anteil %"], result.fgZoneRows)}
  </section>
  <section>
    <h2>Beispiel-Drives</h2>
    ${renderTable(["Quarter", "Off", "Ball", "Dist", "Resolution", "Soft-Fails", "Result", "Summary"], sampleRows)}
  </section>
  <section>
    <h2>Empfehlung</h2>
  ...
  const reportPath = join(reportDir, `qa-aggressive-opp45-to-35-softfail-report-${REPORT_DATE}.html`);
    ...
    `${JSON.stringify(
      {
        ...
        targetedSuccessRate: round(result.targetedSuccessRate * 100, 1),
        softFailRate: round(result.softFailRate * 100, 1),
      },
      null,
      2,
    )}\n`,
