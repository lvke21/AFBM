...
import type { CompetitionRuleset } from "../../src/modules/gameplay/domain/competition-rules";
import type {
...
import type {
  ...
  SideSelectionTrace,
} from "../../src/modules/gameplay/domain/play-selection";
...
import {
  ...
  type PassPlayDevelopmentResolution,
} from "../../src/modules/gameplay/application/play-development-engine";
import { buildPreSnapStructureForPlay } from "../../src/modules/gameplay/application/play-library-service";
import {
  DefaultPlaySelectionEngine,
  createSelectionStrategyProfile,
} from "../../src/modules/gameplay/application/play-selection-engine";
...
import { PLAY_LIBRARY_CATALOG, buildDefaultPlaybook } from "../../src/modules/gameplay/infrastructure";
import {
...
function routeTarget(input: {
  id: string;
  label?: string;
  routeType: PassRouteDevelopmentTarget["routeType"];
  routeDepth: PassRouteDevelopmentTarget["routeDepth"];
  baseYards: number;
  receiver?: Partial<PassRouteDevelopmentTarget["receiver"]>;
  defender?: Partial<PassRouteDevelopmentTarget["defender"]>;
}): PassRouteDevelopmentTarget {
  return {
    id: input.id,
...
function summarizePassDevelopmentOutcomes(
  outcomes: PassPlayDevelopmentResolution[],
): OutcomeRates & {
  completionRate: number;
  interceptionRate: number;
  avgDecisionTime: number;
  avgPressureLevel: number;
  avgOpenBlitzLaneRisk: number;
} {
  return {
    passAttemptRate: rate(
      outcomes.filter((outcome) => outcome.decisionOutcome === "PASS_ATTEMPT").length,
      outcomes.length,
    ),
    throwawayRate: rate(
      outcomes.filter((outcome) => outcome.decisionOutcome === "THROWAWAY").length,
      outcomes.length,
    ),
    scrambleRate: rate(
      outcomes.filter((outcome) => outcome.decisionOutcome === "SCRAMBLE").length,
      outcomes.length,
    ),
    sackRate: rate(
      outcomes.filter((outcome) => outcome.decisionOutcome === "SACK").length,
      outcomes.length,
    ),
    sackFumbleRate: rate(
      outcomes.filter((outcome) => outcome.decisionOutcome === "SACK_FUMBLE").length,
      outcomes.length,
    ),
    completionRate: rate(
      outcomes.filter((outcome) => outcome.passOutcome === "COMPLETE").length,
      outcomes.length,
    ),
    interceptionRate: rate(
      outcomes.filter((outcome) => outcome.passOutcome === "INTERCEPTION").length,
      outcomes.length,
    ),
    avgDecisionTime: round(average(outcomes.map((outcome) => outcome.decisionTime)), 3),
    avgPressureLevel: round(average(outcomes.map((outcome) => outcome.pressureLevel)), 3),
    avgOpenBlitzLaneRisk: round(
      average(outcomes.map((outcome) => outcome.openBlitzLaneRisk)),
      3,
    ),
  };
}

function requireCatalogOffensePlay(playId: string): OffensivePlayDefinition {
...
function createSelectionSituation(
...
function createSelectionContext(
  ...
  const situation = createSelectionSituation(ruleset, situationOverrides);
  const offensePlaybook = buildDefaultPlaybook({ teamId: "TEAM-A", ruleset });
  const defensePlaybook = buildDefaultPlaybook({ teamId: "TEAM-B", ruleset });
  const basePreSnap = buildPreSnapStructureForPlay(
    requireCatalogOffensePlay("off-zone-inside-split"),
    situation,
  );
  ...
  return {
    ...
    selectionSeed: "qa-extended-engine-selection",
    ...restOverrides,
    situation,
  };
}
...
function runExtendedUnitTests(): SpecialTestResult[] {
  const passTargets = [
    routeTarget({ id: "quick", label: "Quick Slant", routeType: "QUICK_SLANT", routeDepth: "SHORT", baseYards: 5 }),
    routeTarget({ id: "dig", label: "Dig", routeType: "DIG", routeDepth: "MEDIUM", baseYards: 12 }),
    routeTarget({ id: "go", label: "Go", routeType: "GO", routeDepth: "DEEP", baseYards: 28 }),
  ];
  const balancedQuarterback = {
    POCKET_PRESENCE: 76,
    AWARENESS: 76,
    DECISION_MAKING: 76,
    COMMAND: 76,
  ...
  const fiveVsFour = resolvePassPlayDevelopment({
    offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 9 }),
    defensePlay: defensePlay({ family: "MAN_COVERAGE", pressureCodes: [] }),
    offensiveLine: balancedLine,
    passRush: balancedRush,
    quarterback: balancedQuarterback,
    targets: passTargets,
    decisionSeed: "unit-five-vs-four",
    pressSeed: "unit-five-vs-four",
    matchupSeed: "unit-five-vs-four",
    random: () => 0.42,
  });
  const fiveVsFive = resolvePassPlayDevelopment({
    offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 9 }),
    defensePlay: defensePlay({ family: "FIRE_ZONE", pressureCodes: ["MLB"] }),
    offensiveLine: balancedLine,
    passRush: balancedRush,
    quarterback: balancedQuarterback,
    targets: passTargets,
    decisionSeed: "unit-five-vs-five",
    pressSeed: "unit-five-vs-five",
    matchupSeed: "unit-five-vs-five",
    random: () => 0.42,
  });
  const fiveVsSix = resolvePassPlayDevelopment({
    offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 9 }),
    defensePlay: defensePlay({ family: "ZERO_PRESSURE", pressureCodes: ["MLB", "LOLB"] }),
    offensiveLine: balancedLine,
    passRush: balancedRush,
    quarterback: balancedQuarterback,
    targets: passTargets,
    decisionSeed: "unit-five-vs-six",
    pressSeed: "unit-five-vs-six",
    matchupSeed: "unit-five-vs-six",
    random: () => 0.42,
  });
  const blockerRusherCheck =
    fiveVsFour.blockerCount === 5 &&
    fiveVsFour.rusherCount === 4 &&
    fiveVsFive.rusherCount === 5 &&
    fiveVsSix.rusherCount === 6 &&
    fiveVsFour.doubleTeamChance > fiveVsFive.doubleTeamChance &&
    fiveVsSix.openBlitzLaneRisk > fiveVsFive.openBlitzLaneRisk &&
    fiveVsSix.pressureLevel > fiveVsFour.pressureLevel;

  const blitzDefense = defensePlay({ family: "ZERO_PRESSURE", pressureCodes: ["MLB", "LOLB"] });
  const deepAgainstBlitz = Array.from({ length: SPECIAL_TEST_ITERATIONS }, (_, index) =>
    resolvePassPlayDevelopment({
      offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 16 }),
      defensePlay: blitzDefense,
      offensiveLine: { PASS_BLOCK: 70, FOOTWORK: 70, ANCHOR: 70, HAND_TECHNIQUE: 70 },
      passRush: { PASS_RUSH: 84, POWER_MOVES: 84, FINESSE_MOVES: 82, BLOCK_SHEDDING: 82 },
      quarterback: balancedQuarterback,
      targets: passTargets,
      decisionSeed: `unit-blitz-deep-${index + 1}`,
      pressSeed: `unit-blitz-deep-${index + 1}`,
      matchupSeed: `unit-blitz-deep-${index + 1}`,
    }),
  );
  const screenAgainstBlitz = Array.from({ length: SPECIAL_TEST_ITERATIONS }, (_, index) =>
    resolvePassPlayDevelopment({
      ...
      decisionSeed: `unit-blitz-screen-${index + 1}`,
      pressSeed: `unit-blitz-screen-${index + 1}`,
      matchupSeed: `unit-blitz-screen-${index + 1}`,
    }),
  );
  const deepBlitzRates = summarizePassDevelopmentOutcomes(deepAgainstBlitz);
  const screenBlitzRates = summarizePassDevelopmentOutcomes(screenAgainstBlitz);
  const blitzCounterCheck =
    screenBlitzRates.passAttemptRate > deepBlitzRates.passAttemptRate &&
    screenBlitzRates.sackRate <= deepBlitzRates.sackRate &&
    screenBlitzRates.avgPressureLevel < deepBlitzRates.avgPressureLevel;

  const elitePressVsWeakWr = resolvePassPlayDevelopment({
    offensePlay: offensePlay({ family: "QUICK_PASS", expectedYards: 4 }),
    defensePlay: defensePlay({ family: "MAN_COVERAGE", pressureCodes: [] }),
    offensiveLine: balancedLine,
    passRush: balancedRush,
    quarterback: balancedQuarterback,
    ...
    decisionSeed: "unit-elite-press",
    pressSeed: "unit-elite-press",
    matchupSeed: "unit-elite-press",
    random: () => 0.38,
  });
  const weakPressVsEliteWr = resolvePassPlayDevelopment({
    offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 18 }),
    defensePlay: defensePlay({ family: "MAN_COVERAGE", pressureCodes: [] }),
    offensiveLine: balancedLine,
    passRush: balancedRush,
    quarterback: { ...balancedQuarterback, COMMAND: 84, DECISION_MAKING: 84 },
    targets: [
      routeTarget({
        id: "elite-wr",
        label: "Elite WR Go",
    ...
    decisionSeed: "unit-weak-press",
    pressSeed: "unit-weak-press",
    matchupSeed: "unit-weak-press",
    random: () => 0.68,
  });
  const elitePressResult = elitePressVsWeakWr.press[0]?.press;
  const weakPressResult = weakPressVsEliteWr.press[0]?.press;
  const pressCheck =
    elitePressResult != null &&
    weakPressResult != null &&
    elitePressResult.timingDisruption > weakPressResult.timingDisruption &&
    weakPressResult.bigPlayRiskDelta > elitePressResult.bigPlayRiskDelta;

  const stableLine = ["lt", "lg", "c", "rg", "rt"];
  const stableChemistry = buildChemistryState({
    weeks: 10,
    usagesForWeek: () => [
      { type: "OFFENSIVE_LINE", teamId: "CHEM", playerIds: stableLine, snaps: 66 },
    ],
  });
  const unstableChemistry = buildChemistryState({
    weeks: 10,
    usagesForWeek: (week) => [
      {
        type: "OFFENSIVE_LINE",
        teamId: "CHEM",
        playerIds: ["lt", `lg-${week}`, "c", `rg-${week}`, `rt-${week}`],
        snaps: 66,
      },
    ],
  });
  const stableChemistryModifiers = calculateUnitChemistryModifiers({
    state: stableChemistry,
    teamId: "CHEM",
    offensiveLinePlayerIds: stableLine,
  });
  const unstableChemistryModifiers = calculateUnitChemistryModifiers({
    state: unstableChemistry,
    teamId: "CHEM",
    offensiveLinePlayerIds: ["lt", "lg-10", "c", "rg-10", "rt-10"],
  });
  const stableProtection = resolvePassPlayDevelopment({
    offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 10 }),
    defensePlay: defensePlay({ family: "FIRE_ZONE", pressureCodes: ["MLB"] }),
    offensiveLine: { PASS_BLOCK: 72, FOOTWORK: 72, ANCHOR: 72, HAND_TECHNIQUE: 72 },
    passRush: { PASS_RUSH: 80, POWER_MOVES: 80, FINESSE_MOVES: 78, BLOCK_SHEDDING: 80 },
    quarterback: balancedQuarterback,
    chemistryModifier: {
      passProtectionBonus: stableChemistryModifiers.offensiveLine.passProtectionBonus,
      qbReceiver: stableChemistryModifiers.qbReceiver,
      defensiveBack: stableChemistryModifiers.defensiveBack,
    },
    targets: passTargets,
    decisionSeed: "unit-stable-chemistry",
  });
  const unstableProtection = resolvePassPlayDevelopment({
    offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 10 }),
    defensePlay: defensePlay({ family: "FIRE_ZONE", pressureCodes: ["MLB"] }),
    offensiveLine: { PASS_BLOCK: 72, FOOTWORK: 72, ANCHOR: 72, HAND_TECHNIQUE: 72 },
    passRush: { PASS_RUSH: 80, POWER_MOVES: 80, FINESSE_MOVES: 78, BLOCK_SHEDDING: 80 },
    quarterback: balancedQuarterback,
    chemistryModifier: {
      passProtectionBonus: unstableChemistryModifiers.offensiveLine.passProtectionBonus,
      qbReceiver: unstableChemistryModifiers.qbReceiver,
      defensiveBack: unstableChemistryModifiers.defensiveBack,
    },
    targets: passTargets,
    decisionSeed: "unit-unstable-chemistry",
  });
  const chemistryCheck =
    stableChemistryModifiers.offensiveLine.passProtectionBonus >
      unstableChemistryModifiers.offensiveLine.passProtectionBonus &&
    stableChemistryModifiers.offensiveLine.chemistry >
      unstableChemistryModifiers.offensiveLine.chemistry;

  const highCommandSeries = Array.from({ length: SPECIAL_TEST_ITERATIONS }, (_, index) =>
    resolvePassPlayDevelopment({
      offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 12 }),
      defensePlay: blitzDefense,
      offensiveLine: { PASS_BLOCK: 66, FOOTWORK: 66, ANCHOR: 66, HAND_TECHNIQUE: 66 },
      passRush: { PASS_RUSH: 83, POWER_MOVES: 82, FINESSE_MOVES: 84, BLOCK_SHEDDING: 82 },
      quarterback: {
        POCKET_PRESENCE: 90,
        AWARENESS: 90,
        DECISION_MAKING: 90,
      ...
      targets: passTargets,
      decisionSeed: `unit-command-high-${index + 1}`,
    }),
  );
  const lowCommandSeries = Array.from({ length: SPECIAL_TEST_ITERATIONS }, (_, index) =>
    resolvePassPlayDevelopment({
      offensePlay: offensePlay({ family: "DROPBACK", expectedYards: 12 }),
      defensePlay: blitzDefense,
      offensiveLine: { PASS_BLOCK: 66, FOOTWORK: 66, ANCHOR: 66, HAND_TECHNIQUE: 66 },
      ...
      targets: passTargets,
      decisionSeed: `unit-command-low-${index + 1}`,
    }),
  );
  const highCommandRates = summarizePassDevelopmentOutcomes(highCommandSeries);
  const lowCommandRates = summarizePassDevelopmentOutcomes(lowCommandSeries);
  const commandCheck =
    highCommandRates.avgDecisionTime > lowCommandRates.avgDecisionTime &&
    highCommandRates.sackRate < lowCommandRates.sackRate;

  const runDefenseTest = resolveRunPlayDevelopment({
    offensePlay: offensePlay({
      family: "GAP_RUN",
      extraPlayers: [player("TE", 7), player("FB", 8)],
    }),
  ...
  const runDefenseCheck =
    runDefenseTest.interpretation === "DEFENSE_ADVANTAGE" &&
    runDefenseTest.negativeYardageRisk > 0.18 &&
    runDefenseTest.laneQuality < 50;

  const selectionEngine = new DefaultPlaySelectionEngine();
  const coordinatorBaseline = selectionEngine.select(
    createSelectionContext({
      selectionSeed: "qa-coordinator-baseline",
      situation: { down: 2, yardsToGo: 9, distanceBucket: "MEDIUM", fieldZone: "MIDFIELD" },
      opponentWeaknessForOffense: { passRush: 88 },
      opponentWeaknessForDefense: { passProtection: 72 },
    }),
  );
  const coordinatorGameplan = selectionEngine.select(
    createSelectionContext({
      selectionSeed: "qa-coordinator-gameplan",
      situation: { down: 2, yardsToGo: 9, distanceBucket: "MEDIUM", fieldZone: "MIDFIELD" },
      offenseCoordinator: { screenQuickVsBlitz: 0.94, aggressiveness: -0.2, runPassLean: 0.22 },
      defenseCoordinator: { blitzFrequency: 0.92, pressFrequency: 0.78, aggressiveness: 0.72 },
      offenseXFactorPlan: { protectionPlan: "FAST_RELEASE", offensiveMatchupFocus: "PROTECT_QB" },
      defenseXFactorPlan: {
      ...
      opponentWeaknessForOffense: { passRush: 88 },
      opponentWeaknessForDefense: { passProtection: 58, quarterbackDecision: 60 },
      offenseTeamStrength: { passProtection: 62, passOffense: 78 },
      defenseTeamStrength: { passRush: 86, coverage: 82 },
    }),
  );
  const baselineQuickScreen = combinedFamilyProbability(coordinatorBaseline.trace.offense, [
    "SCREEN",
    "QUICK_PASS",
    "EMPTY_TEMPO",
  ]);
  const gameplanQuickScreen = combinedFamilyProbability(coordinatorGameplan.trace.offense, [
    "SCREEN",
    "QUICK_PASS",
    "EMPTY_TEMPO",
  ]);
  const baselinePressure = combinedFamilyProbability(coordinatorBaseline.trace.defense, [
    "FIRE_ZONE",
    "ZERO_PRESSURE",
    "SIMULATED_PRESSURE",
    "RUN_BLITZ",
  ]);
  const gameplanPressure = combinedFamilyProbability(coordinatorGameplan.trace.defense, [
    "FIRE_ZONE",
    "ZERO_PRESSURE",
    "SIMULATED_PRESSURE",
    "RUN_BLITZ",
  ...
  const coordinatorCheck =
    gameplanQuickScreen > baselineQuickScreen && gameplanPressure > baselinePressure;
  ...
  const goodCoach = resolveHeadCoachMomentum({
    ...
    recentEvents: [
      { type: "SACK", sequence: 1 },
      { type: "FUMBLE", sequence: 2 },
      { type: "BIG_PLAY_AGAINST", sequence: 3 },
    ],
  ...
  const headCoachCheck =
    goodCoach.errorChainRisk < poorCoach.errorChainRisk &&
    goodCoach.collapseRisk < poorCoach.collapseRisk;

  return [
    {
      id: "unit-5-blocker-vs-4-rusher",
      label: "5 Blocker vs 4 Rusher",
      category: "Unit Test · Pass Protection",
      iterations: 1,
      metrics: {
        blockerCount: fiveVsFour.blockerCount,
        rusherCount: fiveVsFour.rusherCount,
        protectionAdvantage: fiveVsFour.protectionAdvantage,
        doubleTeamChance: fiveVsFour.doubleTeamChance,
        pressureLevel: fiveVsFour.pressureLevel,
        pocketStability: fiveVsFour.pocketStability,
      },
      verdict: blockerRusherCheck ? "GRUEN" : "ROT",
      explanation: "Mehr Blocker als Rusher erzeugen Double-Team-Spielraum und niedrigeren Druck.",
    },
    {
      id: "unit-5-blocker-vs-5-rusher",
      label: "5 Blocker vs 5 Rusher",
      category: "Unit Test · Pass Protection",
      iterations: 1,
      metrics: {
        blockerCount: fiveVsFive.blockerCount,
        rusherCount: fiveVsFive.rusherCount,
        protectionAdvantage: fiveVsFive.protectionAdvantage,
        doubleTeamChance: fiveVsFive.doubleTeamChance,
        openBlitzLaneRisk: fiveVsFive.openBlitzLaneRisk,
        pressureLevel: fiveVsFive.pressureLevel,
      },
      verdict: blockerRusherCheck ? "GRUEN" : "ROT",
      explanation: "Gleiche Zahlen neutralisieren Double Teams und halten Pressure im Mittelbereich.",
    },
    {
      id: "unit-5-blocker-vs-6-rusher",
      label: "5 Blocker vs 6 Rusher",
      category: "Unit Test · Pass Protection",
      iterations: 1,
      metrics: {
        blockerCount: fiveVsSix.blockerCount,
        rusherCount: fiveVsSix.rusherCount,
        protectionAdvantage: fiveVsSix.protectionAdvantage,
        openBlitzLaneRisk: fiveVsSix.openBlitzLaneRisk,
        pressureLevel: fiveVsSix.pressureLevel,
        pocketStability: fiveVsSix.pocketStability,
      },
      verdict: blockerRusherCheck ? "GRUEN" : "ROT",
      explanation: "Mehr Rusher als Blocker erzeugen Open-Blitz-Lane-Risiko, aber keinen automatischen Sack.",
    },
    {
      id: "unit-blitz-deep-vs-screen",
      label: "Blitz gegen Deep Pass / Screen",
      category: "Unit Test · Blitz Counter",
      iterations: SPECIAL_TEST_ITERATIONS,
      metrics: {
        deepAvgDecisionTime: deepBlitzRates.avgDecisionTime,
        screenAvgDecisionTime: screenBlitzRates.avgDecisionTime,
        deepPressureLevel: deepBlitzRates.avgPressureLevel,
        screenPressureLevel: screenBlitzRates.avgPressureLevel,
        deepSackRate: deepBlitzRates.sackRate,
        screenSackRate: screenBlitzRates.sackRate,
        screenPassAttemptRate: screenBlitzRates.passAttemptRate,
      },
      verdict: blitzCounterCheck ? "GRUEN" : "ROT",
      explanation: "Screen/Quick Timing ist ein sichtbarer Blitz-Counter, gewinnt aber nicht automatisch.",
    },
    {
      id: "unit-press-matchups",
      label: "Elite Press CB vs schwacher WR / schwacher Press CB vs Elite Release WR",
      category: "Unit Test · Press Coverage",
      iterations: 2,
      metrics: {
        elitePressOutcome: elitePressResult?.outcome ?? "NONE",
        elitePressTimingDisruption: elitePressResult?.timingDisruption ?? 0,
        elitePressSeparationDelta: elitePressResult?.separationDelta ?? 0,
        weakPressOutcome: weakPressResult?.outcome ?? "NONE",
        weakPressTimingDisruption: weakPressResult?.timingDisruption ?? 0,
        weakPressBigPlayRiskDelta: weakPressResult?.bigPlayRiskDelta ?? 0,
      },
      verdict: pressCheck ? "GRUEN" : "ROT",
      explanation: "Press stoert Timing sichtbar, kann bei verlorenem Release aber Big-Play-Risiko erzeugen.",
    },
    {
      id: "unit-good-vs-bad-ol-chemistry",
      label: "Gute O-Line Chemistry vs schlechte O-Line Chemistry",
      category: "Unit Test · Chemistry",
      iterations: 10,
      metrics: {
        stableLineChemistry: stableChemistryModifiers.offensiveLine.chemistry,
        unstableLineChemistry: unstableChemistryModifiers.offensiveLine.chemistry,
        stablePassProtectionBonus: stableChemistryModifiers.offensiveLine.passProtectionBonus,
        unstablePassProtectionBonus: unstableChemistryModifiers.offensiveLine.passProtectionBonus,
        stablePressureLevel: stableProtection.pressureLevel,
        unstablePressureLevel: unstableProtection.pressureLevel,
        pressureDelta: round(unstableProtection.pressureLevel - stableProtection.pressureLevel, 3),
      },
      verdict: chemistryCheck ? "GRUEN" : "ROT",
      explanation: "Chemistry differenziert die Protection-Modifier messbar; der Effekt bleibt klein und erzeugt keinen dominanten Pressure-Swing.",
    },
    {
      id: "unit-good-vs-bad-qb-command",
      label: "Guter QB Command vs schlechter QB Command",
      category: "Unit Test · QB Decision",
      iterations: SPECIAL_TEST_ITERATIONS,
      metrics: {
        highCommandAvgDecisionTime: highCommandRates.avgDecisionTime,
        lowCommandAvgDecisionTime: lowCommandRates.avgDecisionTime,
        highCommandThrowawayRate: highCommandRates.throwawayRate,
        lowCommandThrowawayRate: lowCommandRates.throwawayRate,
        highCommandScrambleRate: highCommandRates.scrambleRate,
        lowCommandScrambleRate: lowCommandRates.scrambleRate,
        highCommandSackRate: highCommandRates.sackRate,
        lowCommandSackRate: lowCommandRates.sackRate,
      },
      verdict: commandCheck ? "GRUEN" : "ROT",
      explanation: "Command/Awareness verbessern die verfuegbare Zeit und senken Katastrophenraten.",
    },
    {
      id: "unit-strong-run-defense-vs-weak-run-block",
      label: "Starke Run Defense vs schwache Run Blocking Unit",
      category: "Unit Test · Run Lane",
      iterations: 1,
      metrics: {
        blockerCount: runDefenseTest.blockerCount,
        boxDefenderCount: runDefenseTest.boxDefenderCount,
        blockerAdvantage: runDefenseTest.blockerAdvantage,
        laneQuality: runDefenseTest.laneQuality,
        stuffedRisk: runDefenseTest.stuffedRisk,
        negativeYardageRisk: runDefenseTest.negativeYardageRisk,
        expectedYardsModifier: runDefenseTest.expectedYardsModifier,
      },
      verdict: runDefenseCheck ? "GRUEN" : "ROT",
      explanation: "Defensive Ueberlegenheit in der Box verschlechtert Lane Quality und erhoeht Negative Runs.",
    },
    {
      id: "unit-coordinator-gameplan-impact",
      label: "Coordinator/Gameplan Impact",
      category: "Unit Test · Coordinator",
      iterations: 2,
      metrics: {
        baselineQuickScreenProbability: round(baselineQuickScreen, 4),
        gameplanQuickScreenProbability: round(gameplanQuickScreen, 4),
        quickScreenDelta: round(gameplanQuickScreen - baselineQuickScreen, 4),
        baselinePressureProbability: round(baselinePressure, 4),
        gameplanPressureProbability: round(gameplanPressure, 4),
        pressureDelta: round(gameplanPressure - baselinePressure, 4),
      },
      verdict: coordinatorCheck ? "GRUEN" : "ROT",
      explanation: "Coordinator- und Gameplan-Tendenzen veraendern Playcalling-Wahrscheinlichkeiten messbar.",
    },
    {
      id: "unit-head-coach-modifier-impact",
      label: "HC Modifier Impact",
      category: "Unit Test · Head Coach",
      iterations: 1,
      metrics: {
        goodCoachFocusModifier: goodCoach.focusModifier,
        poorCoachFocusModifier: poorCoach.focusModifier,
        goodCoachErrorChainRisk: goodCoach.errorChainRisk,
        poorCoachErrorChainRisk: poorCoach.errorChainRisk,
        goodCoachCollapseRisk: goodCoach.collapseRisk,
        poorCoachCollapseRisk: poorCoach.collapseRisk,
      },
      verdict: headCoachCheck ? "GRUEN" : "ROT",
      explanation: "Head Coach stabilisiert nach negativen Plays leicht, ohne Comebacks zu erzwingen.",
    },
  ];
}
...
<head>
  <meta charset="utf-8" />
  <title>QA Extended Game Engine Balance Report</title>
  <style>
...
<body>
  <header>
    <h1>QA Extended Game Engine Balance Report</h1>
  ...
  <main>
    <section>
      ...
      <p>Die erweiterte Engine wurde mit einer 8-Team-Liga, den geforderten Unit-Szenarien und isolierten Spezialtests fuer Pass Rush, Press Coverage, Blitz-Counter, Run Lanes, QB Command, Chemistry, Coordinator/Gameplan und Head Coach Modifier gemessen.</p>
...
function writeReports() {
  ...
  const specialTests = [...runSpecialTests(), ...runExtendedUnitTests()];
  const statusChecks = {
  ...
  writeFileSync(
    join(reportsDir, "qa-extended-engine-balance-results.json"),
    `${JSON.stringify(payload, null, 2)}\n`,
    "utf8",
  );
  writeFileSync(
    join(reportsDir, "qa-extended-engine-balance-report.html"),
  ...
  JSON.stringify(
    {
      ...
      json: "reports-output/simulations/qa-extended-engine-balance-results.json",
      html: "reports-output/simulations/qa-extended-engine-balance-report.html",
    },
    null,
    2,
  ),
